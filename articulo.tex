...
\section{Script de Automatización de Configuración y Compilación (setup.sh)}
\label{ap:setup_sh}
...
\label{ap:runners_sh}
\begin{lstlisting}[language=bash,caption={\small Script lanzador de Gazebo Classic (launch\_gazebo.sh).}]
#!/bin/bash
SCRIPT_DIR="$( cd "$( dirname "${BASH_SOURCE[0]}" )" &> /dev/null && pwd )"
